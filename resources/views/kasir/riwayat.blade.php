...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
    <div class="bg-white rounded-xl border border-gray-200 p-5 flex items-center justify-between">
        ...
        <div class="w-10 h-10 rounded-full bg-brand-100 flex items-center justify-center flex-shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-brand-700"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z"/></svg>
        </div>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 p-5 flex items-center justify-between">
        <div>
...
<div class="bg-white rounded-xl border border-gray-200 p-4 mb-6">
    <div class="flex flex-wrap items-center justify-end gap-3">
        <form method="GET" action="{{ url('/kasir/riwayat') }}" class="flex items-center gap-2 w-full sm:w-auto">
            <input type="date" name="date" value="{{ $filterDate }}" class="flex-1 sm:flex-none px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-brand-400 transition-all bg-white cursor-pointer">
    ...
    <table class="w-full" id="riwayat-table">
        <thead>
            <tr class="bg-gray-50 border-b border-gray-200">
                ...
                <th class="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide hidden sm:table-cell">Tanggal</th>
                ...
                <th class="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide hidden sm:table-cell">Item</th>
                <th class="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide hidden md:table-cell">Status</th>
        ...
        <tbody>
            @forelse($transaksi as $t)
                <tr class="riwayat-row border-b border-gray-100 cursor-pointer transition-colors hover:bg-slate-50"
                    ...
                    <td class="px-5 py-3.5 text-sm text-gray-600 hidden sm:table-cell">{{ $t->created_at->format('d/m/Y H:i') }}</td>
                    ...
                    <td class="px-5 py-3.5 text-sm text-gray-500 hidden sm:table-cell">{{ $t->total_item }} item</td>
                    <td class="px-5 py-3.5 hidden md:table-cell">
                        ...
                        @elseif($t->status_pembayaran === 'Menunggu Validasi')
                            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-accent-50 text-accent-700 border border-accent-200">
                                <span class="w-1.5 h-1.5 rounded-full bg-accent-500"></span> Menunggu
                            </span>
                    ...
                    <td class="px-5 py-3.5 text-center" onclick="event.stopPropagation();">
                        <div class="flex items-center justify-center gap-1.5">
                            ...
                            @if($t->bukti_pembayaran)
                                <a href="{{ url('/bukti-pembayaran/' . basename($t->bukti_pembayaran)) }}" target="_blank" class="inline-flex items-center gap-1 px-2 py-1 sm:px-2.5 sm:py-1.5 rounded-lg text-xs font-semibold text-gray-600 bg-gray-50 hover:bg-gray-100 no-underline transition-all border border-gray-200">
                            ...
                            <a href="{{ url('/transaksi/'.$t->id.'/invoice-pdf') }}" target="_blank"
                               ...
                               title="{{ $t->status_pembayaran === 'Sudah Dibayar' ? 'Cetak Invoice LUNAS (PDF)' : 'Cetak Invoice (PDF)' }}">
