...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 items-stretch">
    ...
    <div class="bg-white rounded-2xl p-6 border border-slate-200 shadow-[0_2px_10px_rgba(0,0,0,0.02)] flex items-center justify-between group hover:border-slate-300 transition-colors min-h-[110px]">
        <div>
            <span class="text-slate-500 text-[0.85rem] font-medium tracking-wide">Penjualan Hari Ini</span>
            <div class="flex items-center gap-2 mt-1">
                <h3 class="font-bold text-2xl text-slate-800">Rp {{ number_format($totalPenjualan, 0, ',', '.') }}</h3>
            </div>
        </div>
        <div class="w-12 h-12 rounded-xl flex items-center justify-center bg-emerald-50 text-emerald-600 group-hover:scale-110 transition-transform duration-300 flex-shrink-0">
    ...
    <div class="bg-white rounded-2xl p-6 border border-slate-200 shadow-[0_2px_10px_rgba(0,0,0,0.02)] flex items-center justify-between group hover:border-slate-300 transition-colors min-h-[110px]">
        <div>
            <span class="text-slate-500 text-[0.85rem] font-medium tracking-wide">Transaksi Hari Ini</span>
            <h3 class="font-bold text-2xl mt-1 text-slate-800">{{ $totalTransaksi }}</h3>
        </div>
        <div class="w-12 h-12 rounded-xl flex items-center justify-center bg-[#FAF5EF] text-[#7B3911] group-hover:scale-110 transition-transform duration-300 flex-shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
    ...
    {{-- Card 3: Tagihan Belum Dibayar (khusus kasir) --}}
    <a href="{{ url('/kasir/tagihan') }}" class="no-underline block min-h-[110px]">
        <div class="h-full bg-white rounded-2xl p-6 border {{ $tagihanMendesak > 0 ? 'border-red-200 bg-red-50/10' : 'border-slate-200' }} shadow-[0_2px_10px_rgba(0,0,0,0.02)] flex items-center justify-between group hover:border-slate-300 transition-colors relative overflow-hidden">
            @if($tagihanMendesak > 0)
                <div class="absolute top-0 right-0 w-2 h-full bg-red-500 animate-pulse"></div>
            @endif
            <div>
                <span class="text-slate-500 text-[0.85rem] font-medium tracking-wide">Belum Dibayar</span>
                <h3 class="font-bold text-2xl mt-1 text-slate-800">{{ $belumBayar }}</h3>
                @if($tagihanMendesak > 0)
                ...
                @else
                    <span class="text-[10px] text-transparent font-bold block mt-1 uppercase tracking-wider select-none">-</span>
                @endif
            </div>
    ...
    {{-- Card 4: Total Stok --}}
    <div class="bg-white rounded-2xl p-6 border {{ $isStokRendah ? 'border-red-200 bg-red-50/30' : 'border-slate-200' }} shadow-[0_2px_10px_rgba(0,0,0,0.02)] flex items-center justify-between group hover:border-slate-300 transition-colors relative overflow-hidden min-h-[110px]">
        @if($isStokRendah)
            <div class="absolute top-0 right-0 w-2 h-full bg-red-500 animate-pulse"></div>
        @endif
        <div>
            ...
            @if($isStokRendah)
                <span class="text-[0.7rem] font-bold uppercase tracking-wider bg-red-100 px-2 py-0.5 rounded text-red-700 mt-1.5 inline-block">Stok Tipis ({{ $stokRendahCount }})</span>
            @else
                <span class="text-[0.7rem] font-bold uppercase tracking-wider text-transparent mt-1.5 inline-block select-none">-</span>
            @endif
        </div>
        <div class="w-12 h-12 rounded-xl flex items-center justify-center {{ $isStokRendah ? 'bg-red-100 text-red-600' : 'bg-slate-100 text-slate-600' }} group-hover:scale-110 transition-transform duration-300 flex-shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
<div class="flex justify-between items-center mb-6">
    ...
    <div class="flex items-center gap-3">
        <a href="{{ url('/kasir/transaksi') }}" class="flex items-center gap-2 px-4 py-2 rounded-xl font-semibold text-sm transition-all no-underline" style="background: linear-gradient(135deg,#FAF5EF,#FFF8F0); border: 1.5px solid #E0C4A8; color: #7B3911;">
